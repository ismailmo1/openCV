...
cornerConts = []
cornerCenters = np.array([[0,0]])
# draw all contours on original image
for c in contours:
    # compute the bounding box of the contour to get aspect ratio, x and y values - use this identify omr spots
    ...
    if 0.8 <= ar <= 1.2: #60 >= w >= 40 and 60 >= h >= 40 and  #and ((x > 500 and (y>400 or y<100)) or (x < 50 and (y>400 or y<100))):
        ...
        cv2.drawContours(thresh, c, -1, (255, 255, 255))
        cornerConts.append(c)
        cornerCenters = np.append(cornerCenters, (np.array([[c_x, c_y]])), axis=1)
        #draw on blank to show correct detection
        cv2.circle(imgBlank, (c_x, c_y), 1, (255,0,0), 2)
